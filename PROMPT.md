**MINIONS GRANTS — GRANT & FUNDING MANAGER**

You are tasked with creating the complete initial foundation for **minions-grants** — structured grant and funding management. This is part of the Minions ecosystem, a universal structured object system designed for building AI-native tools.

---

**PROJECT OVERVIEW**

`minions-grants` is a structured system for managing grant applications, funding opportunities, deadlines, budgets, and proposal development.
...
1. **`grant`** — A funding opportunity (Fields: `name` (string, required), `funder` (string, required), `amount` (number), `deadline` (date), `status` (select: identified/applying/submitted/awarded/rejected), `url` (url))
2. **`proposal`** — A grant application (Fields: `title` (string, required), `abstract` (textarea), `budget` (json), `status` (select: draft/review/submitted)) — Relations: `references` → grant
3. **`deadline`** — A key date (Fields: `name` (string, required), `date` (date, required), `type` (select: application/report/milestone)) — Relations: `references` → grant
4. **`budget-item`** — A line item in a proposal budget (Fields: `category` (string, required), `amount` (number, required), `justification` (textarea)) — Relations: `part_of` → proposal
5. **`funder`** — A funding organization (Fields: `name` (string, required), `website` (url), `focus` (tags), `contactEmail` (email))
6. **`review`** — Feedback on a proposal (Fields: `reviewer` (string), `score` (number), `comments` (textarea), `decision` (select)) — Relations: `references` → proposal

---

**BEYOND STANDARD PATTERN**

- Deadline dashboard with countdown timers
...
```bash
grants deadlines
grants proposal draft <grant-id>
grants budget <proposal-id>
grants status
```
...
Agents can track funding opportunities, manage deadlines, and help draft proposals with budget calculations.
...
Complete Python port with identical functionality. Published to PyPI as `minions-grants`.
